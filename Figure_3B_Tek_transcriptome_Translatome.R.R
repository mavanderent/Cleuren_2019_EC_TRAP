...
### Tek Cre input (transcriptome) vs bead (translatome)
### 09_04_2019
rm(list=ls())
...
dat           <- read.csv("09_04_2019_orginal_all.csv", header = TRUE, row.names = 1)
### samples
samples       <- read.csv("09_04_19_samples_TEK.csv", header = T,row.names = 1)
...
### Plot
### MOCK RANGES
rowRanges <- GRanges(rep(c("chr1"), 14291),
                     IRanges(floor(runif(14291, 1e5, 1e6)), width=100),
                     strand=sample(c("+", "-"), 14291, TRUE),
                     feature_id=sprintf("ID%03d", 1:14291))
...
png("Figure3B.png", width = 5, height = 4, units = "in", res = 600)
pca
dev.off()
rm(list=ls())
