### Ribo analysis
### Comparison of the Tie2_Cre with/without LPS treatmentS
### version 03_30_2019
rm(list=ls())
...
setwd("$PATH")
dat    <- read.csv("02_02_19_original_LPS_RPKM.csv", header = TRUE, row.names = 1)

### load metadata
setwd("$PATH")
info   <- read.csv("02_02_19_original_LPS_samples.csv", header = TRUE, row.names = 1)
...
exp <- exp[IND,]
rm(i,exp2,info2, IND, TPM, noMT, info, tissue, treatment, genotype, DF, DT, n, dat)
...
setwd("$PATH")
write.csv(ratio,"03_30_2019_analysis_LPS_ratio.csv")
rm(n1,p)
...
setwd("$PATH")
write.csv(input,"03_30_2019_analysis_LPS_input.csv")
rm(n1,p)
...
setwd("$PATH")
write.csv(bead,"03_30_2019_analysis_LPS_bead.csv")
rm(n1,p)
